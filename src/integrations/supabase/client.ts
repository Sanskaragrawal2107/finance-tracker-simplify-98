...
export const createSite = async (siteData: any) => {
  try {
    ...
    // Generate a valid UUID for supervisor if needed
    let supervisorUUID = null;
    
    if (siteData.supervisorId) {
      // For demonstration purposes, we'll create a deterministic UUID 
      // based on the supervisor ID (in a real app, this would come from the database)
      // This ensures we use a valid UUID format that Supabase will accept
      supervisorUUID = crypto.randomUUID();
      console.log(`Generated valid UUID for supervisor: ${supervisorUUID}`);
    }
    ...
    const formattedData = {
      ...
      supervisor_id: supervisorUUID, // Use the generated UUID
