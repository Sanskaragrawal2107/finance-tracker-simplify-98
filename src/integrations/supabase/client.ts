...
export const fetchSiteInvoices = async (siteId: string) => {
  ...
  try {
    ...
    return data.map(invoice => {
      ...
      try {
        if (invoice.material_items === null) {
          materialItems = [];
        }
        // Check if material_items is already an object (not a string)
          ...
          if (Array.isArray(invoice.material_items)) {
            ...
            materialItems = invoice.material_items.map((item: any) => ({
          ...
          if (Array.isArray(parsedItems)) {
            materialItems = parsedItems.map((item: any) => ({
              id: item.id || undefined,
              material: item.material || '',
              quantity: typeof item.quantity === 'number' ? item.quantity : null,
              rate: typeof item.rate === 'number' ? item.rate : null,
      ...
      try {
        if (invoice.bank_details === null) {
          // Keep default bankDetails
        }
